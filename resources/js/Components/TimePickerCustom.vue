...
<script>
    ...
    export default defineComponent({
        ...
        setup(props) {
            //console.log(props.filters.times)
            // Generate array of hours
            const hoursArray = computed(() => {
                const arr = [];
                for (let i = 0; i < 24; i++) {
                    if( (i % props.hoursIncrement) === 0 ) { // Check ค่า increment
                        if(!props.filters.times.hours.includes(i)) { // Check ค่าต้องไม่มีอยู่ใน props filter ที่กำหนดไว้
            ...
            const minutesArray = computed(() => {
                const arr = [];
                for (let i = 0; i < 60; i++) {
                    if( (i % props.minutesIncrement) === 0 ) { // Check ค่า increment
                        if(!props.filters.times.minutes.includes(i)) { // Check ค่าต้องไม่มีอยู่ใน props filter ที่กำหนดไว้
                            arr.push({ text: i < 10 ? `0${i}` : i, value: i });
                        }
                    }
                }
