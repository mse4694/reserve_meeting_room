    ...
    <div class="card">
        ...
        <Dialog v-model:visible="meetingRoomDialog" :style="{width: '450px'}" header="ข้อมูลห้องประชุม" :modal="true" class="p-fluid">
            ...
            <div class="mb-4">
                ...
                <InputText id="room_full_name" v-model.trim="meetingRoom.fullname" required="true" :class="{'p-invalid': submitted && !meetingRoom.fullname}" />
                <small class="p-error" v-if="submitted && !meetingRoom.fullname">จำเป็นต้องใส่ ชื่อเต็มห้องประชุม</small>
            </div>

            <div class="mb-4">
                <label for="room_short_name">ชื่อย่อห้องประชุม</label>
                <InputText id="room_short_name" v-model.trim="meetingRoom.shortname" required="true" :class="{'p-invalid': submitted && !meetingRoom.shortname}" />
                <small class="p-error" v-if="submitted && !meetingRoom.shortname">จำเป็นต้องใส่ ชื่อย่อห้องประชุม</small>
            </div>
...
export default {
    ...
    setup() {
        onMounted(() => {
            ...
            //meetingRoomService.value.getAllRoom().then(data => console.log(data));
            //console.log(meetingRooms.value.length);
        })
        ...
        const meetingRoom = useForm({
            room_full_name: null,
            room_short_name: null,
            building_id: null,
            floor: null,
            status: null,
        ...
        const saveMeetingRoom = () => {
            ...
            //    )) {
            if(! meetingRoom.fullname ) {
                console.log('meetingRoom Object is missing some value');
            } else {
                if(meetingRoom.id) {
                    meetingRoom.status = meetingRoom.status.value ? meetingRoom.status.value : meetingRoom.status;
                    meetingRoom.building_id = meetingRoom.building_id.building_id ? meetingRoom.building_id.building_id : meetingRoom.building_id;
                    ...
                    if(typeof meetingRoom.building_id === "object") {
                        ...
                        // ส่งข้อมูลไปที่ controller โดยทำการแปลงข้อมูลบางอย่างจากที่เรารับมาจาก form ก่อนส่งไป ซึ่งถ้า ส่วนไหนที่ไม่มีการแปลง ก็ไม่ต้องกำหนดค่าให้ใหม่
                        meetingRoom.transform(data => ({
                            ...data,
        ...
        const editMeetingRoom = (mRoom) => {
            ...
            meetingRoom.id = tempRoom.id
            meetingRoom.fullname = tempRoom.fullname
            meetingRoom.shortname = tempRoom.shortname
            meetingRoom.building_id = tempRoom.building_id
            meetingRoom.floor = tempRoom.floor
            meetingRoom.capacity_normal = tempRoom.capacity_normal
            meetingRoom.capacity_min = tempRoom.capacity_min
            meetingRoom.capacity_max = tempRoom.capacity_max
            meetingRoom.price_half_day = tempRoom.price_half_day
            meetingRoom.price_full_day = tempRoom.price_full_day
            meetingRoom.status = tempRoom.status
            meetingRoom.description = tempRoom.description
            meetingRoom.image = tempRoom.image
